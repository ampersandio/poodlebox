...
def get_certificates(student_id):
    data=read_query("select id,course_id,issued_date from certificates where users_id=? group by id",(student_id,))
    if data==[]:
        return []
...
def get_certificate_by_course(student_id,course_id):
    data=read_query("select id,course_id,issued_date from certificates where users_id=? and course_id=? group by id",(student_id,course_id))
    if data==[]:
        return None
    return (Certificate.read_from_query_result(*row)for row in data)  
