...
def get_all_callendars_teacher(teacher_id):
    ...
    courses = read_query(
        "select group_concat(distinct id) from courses where owner=?", (teacher_id,)
    )
    if courses == [(None,)]:
        return []
    list_of_courses = [int(x) for x in courses[0][0].split(",")]
...
def get_calendar_by_id(calendar_id):
    ...
    calendar_and_teacher_data = read_query(
        "select c.id,c.name,c.course_id,u.id,u.first_name,u.last_name,u.phone_number,u.email,u.linked_in_profile from calendars c join users u on c.owner=u.id where c.id=?",
        (calendar_id,),
    )
    if calendar_and_teacher_data == []:
        return None
    owner = TeacherShow.read_from_query_result(*calendar_and_teacher_data[0][3:])
    event_data = read_query(
        "select id,name,start,end,link from events where calendar_id=?",
        (calendar_and_teacher_data[0][0],),
    )
    events = []
    for x in event_data:
        event = Event.read_from_query_result(*x)
        events.append(event)
    calendar = CalendarById.read_from_query_result(
        *calendar_and_teacher_data[0][:3], owner=owner, events=events
    )
    return calendar
